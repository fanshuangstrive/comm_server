...
/*
  内部系统与转发器
    通讯协议：socket 流式报文
    报文格式：8位长度PKGHEAD_LENGTH(不足时左补零)$响应码$Tuxedo交易码$数据包$
   	    包头：8位长度
   	    包体：  $响应码$Tuxedo交易码$数据包$
   	    长度:   包体长度
   	    响应码: 0 通讯成功 1超时 2 tuxedo错误 默认填9
   转发器与自来水公司
      通讯协议：Tuxedo、交易码、数据包	    
*/ 
...
void do_process(void)
{
  ...
  snprintf(cfg_path,sizeof(cfg_path),"%s/%s",getenv("HOME"),CFG_FILENAME);	
	...
	snprintf (s_comm_svr_socket.tux_envstr,sizeof(s_comm_svr_socket.tux_envstr),"//%s:%s",s_comm_svr_socket.water_ip,s_comm_svr_socket.water_port);
